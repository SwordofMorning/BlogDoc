...
# 设置红外crop
v4l2-ctl -d /dev/video5 --set-crop top=0,left=0,width=1280,height=512
v4l2-ctl -d /dev/video5 --set-fmt-video=width=1280,height=512,pixelformat=GREY --stream-mmap=3 --stream-to=pic.yuv --stream-count=1
gst-launch-1.0 v4l2src device=/dev/video5 !video/x-raw, format=NV16, width=1280, height=512, framerate=30/1 ! waylandsink
```
