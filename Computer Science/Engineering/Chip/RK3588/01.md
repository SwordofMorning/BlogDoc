...
# 开快门
echo -en '\x43\x44\x4A\x50\x02\x06\x01\x00\x00\x18' > /dev/ttyS4
```

西安：

```sh
v4l2-ctl -d /dev/video0 --set-crop top=0,left=0,width=1280,height=1024
v4l2-ctl -d /dev/video0 --set-fmt-video=width=1280,height=1024,pixelformat=GREY --stream-mmap=10 --stream-to=pic.yuv --stream-count=3 --stream-poll
```
...
# EVF
modetest -M rockchip -s 226@137:1920x1080
```
