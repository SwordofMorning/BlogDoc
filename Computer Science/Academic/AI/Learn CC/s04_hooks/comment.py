...
def permission_hook(block):
    ...
    print(f"\033[90m[HOOK] PreToolUse: permission_hook\033[0m")
...
def log_hook(block):
    ...
    print(f"\033[90m[HOOK] PreToolUse: log_hook\033[0m")
...
def large_output_hook(block, output):
    ...
    print(f"\033[90m[HOOK] PostToolUse: large_output_hook\033[0m")
...
def summary_hook(messages: list):
    print(f"\033[90m[HOOK] Stop: large_output_hook\033[0m")
...
def agent_loop(messages: list):
    while True:
        ...
        if response.stop_reason != "tool_use":
            ...
            # 当前函数永远返回 None；但回调函数中插入单元测试、返回非None时，可以让LLM实现一个“自行检测”的功能
            if force:
