#!/usr/bin/env python3
"""
s09_memory.py - Memory System with Full API & Event Logging

Persistent, cross-session knowledge for the coding agent.
Logs everything to agent.log for debugging.
...
import os, subprocess, json, time, re, datetime
...
# ═══════════════════════════════════════════════════════════
#  Logging Helper: Standardized header style logging
# ═══════════════════════════════════════════════════════════

def log_api_call(tag: str, payload: dict | object, filename: str = "agent.log"):
    """
    Appends LLM API request/response payloads or system events to agent.log.
    """
...
def list_memory_files() -> list[dict]:
    ...
    for f in sorted(MEMORY_DIR.glob("*.md")):
        ...
        result.append({
            ...
            "description": meta.get("description", ""), "type": meta.get("type", "user"), "body": body,
        })
    return result
...
def select_relevant_memories(messages: list, max_items: int = 5) -> list[str]:
    ...
    catalog = "\n".join(catalog_lines)
    prompt = (
        "Given the recent conversation and the memory catalog below, select the indices of memories that are clearly relevant. "
        "Return ONLY a JSON array of integers, e.g. [0, 3]. If none are relevant, return [].\n\n"
        f"Recent conversation:\n{recent}\n\nMemory catalog:\n{catalog}"
    )

    req_payload = {"model": MODEL, "messages": [{"role": "user", "content": prompt}], "max_tokens": 200}
    log_api_call("PRE LLM CALL - SELECT MEMORIES", req_payload)

    try:
        response = client.messages.create(**req_payload)
        log_api_call("POST LLM CALL - SELECT MEMORIES", response)
...
def extract_memories(messages: list):
    ...
    req_payload = {"model": MODEL, "messages": [{"role": "user", "content": prompt}], "max_tokens": 800}
    log_api_call("PRE LLM CALL - EXTRACT MEMORIES", req_payload)

    try:
        response = client.messages.create(**req_payload)
        log_api_call("POST LLM CALL - EXTRACT MEMORIES", response)
        ...
        count = 0
        for mem in items:
            name, mem_type = mem.get("name", f"memory_{int(time.time())}"), mem.get("type", "user")
        ...
        log_api_call("EXTRACT MEMORIES ERROR", {"error": str(e)})
...
def consolidate_memories():
    ...
    prompt = (
        "Consolidate the following memory files. Merge duplicates, remove outdated, keep under 30.\n"
        "Return a JSON array. Each item: {name, type, description, body}.\n\n" + catalog[:16000]
    )
    
    req_payload = {"model": MODEL, "messages": [{"role": "user", "content": prompt}], "max_tokens": 3000}
    log_api_call("PRE LLM CALL - CONSOLIDATE MEMORIES", req_payload)

    try:
        response = client.messages.create(**req_payload)
        log_api_call("POST LLM CALL - CONSOLIDATE MEMORIES", response)
        ...
        for mem in items:
            name, mem_type = mem.get("name", f"memory_{int(time.time())}"), mem.get("type", "user")
            desc, body = mem.get("description", ""), mem.get("body", "")
            if desc and body: write_memory_file(name, mem_type, desc, body)

        print(f"\n\033[33m[Memory: consolidated {len(files)} → {len(items)} memories]\033[0m")
    except Exception as e:
        log_api_call("CONSOLIDATE MEMORIES ERROR", {"error": str(e)})

def build_system() -> str:
    index = read_memory_index()
    memories_section = f"\n\nMemories available:\n{index}" if index else ""
    return (
        f"You are a coding agent at {WORKDIR}.{memories_section}\n"
...
def run_glob(pattern: str) -> str:
    ...
    try:
        ...
        for match in g.glob(pattern, root_dir=WORKDIR):
            if (WORKDIR / match).resolve().is_relative_to(WORKDIR): results.append(match)
...
def spawn_subagent(description: str) -> str:
    ...
    for turn in range(30):
        req_payload = {"model": MODEL, "system": SUB_SYSTEM, "messages": messages, "tools": SUB_TOOLS, "max_tokens": 8000}
...
def _block_type(block): return block.get("type") if isinstance(block, dict) else getattr(block, "type", None)
...
def snip_compact(msgs, mx=50):
    ...
    if head_end > 0 and _message_has_tool_use(msgs[head_end - 1]):
        while head_end < len(msgs) and _is_tool_result_message(msgs[head_end]): head_end += 1
    if (tail_start > 0 and tail_start < len(msgs) and _is_tool_result_message(msgs[tail_start]) and _message_has_tool_use(msgs[tail_start - 1])):
        tail_start -= 1
    if head_end >= tail_start: return msgs
...
def persist_large(tid, out):
    ...
    log_api_call("PERSIST LARGE OUTPUT (L3)", {"tool_use_id": tid, "original_bytes": len(out), "persisted_path": str(p)})
...
def summarize_history(msgs):
    ...
    req_payload = {"model": MODEL, "messages": [{"role": "user", "content": prompt}], "max_tokens": 2000}
    log_api_call("PRE LLM CALL - SUMMARIZE (L4)", req_payload)
    
    r = client.messages.create(**req_payload)
    log_api_call("POST LLM CALL - SUMMARIZE (L4)", r)
    
    return extract_text(r.content).strip()

def compact_history(msgs):
    transcript_path = write_transcript(msgs)
    before_size = estimate_size(msgs)
    summary = summarize_history(msgs)
    compacted = [{"role": "user", "content": f"[Compacted]\n\n{summary}"}]
    
    log_api_call("COMPACT HISTORY (L4 FINISHED)", {
        "transcript_path": str(transcript_path), "before_size": before_size,
        "after_size": estimate_size(compacted), "summary": summary
    })
    return compacted
...
def reactive_compact(msgs):
    ...
    log_api_call("REACTIVE COMPACT (EMERGENCY)", {
        "transcript_path": str(transcript_path), "before_size": before_size, "after_size": estimate_size(compacted)
    })
    return compacted
...
TOOLS = [
    ...
    {"name": "task", "description": "Launch a subagent to handle a subtask.", "input_schema": {"type": "object", "properties": {"description": {"type": "string"}}, "required": ["description"]}},
]
...
def agent_loop(messages: list):
    ...
    # 读取记忆发生在 agent_loop 的最开头，在进入 while True 循环之前。也就是每次用户敲下回车，发送新问题时，只触发一次。参考 14:08:09。
    ...
    while True:
        ...
        if estimate_size(messages) > CONTEXT_LIMIT:
            print("[auto compact]")
            log_api_call("AUTO COMPACT TRIGGERED", {"size_before": estimate_size(messages)})
            messages[:] = compact_history(messages)

        try:
            request_messages = messages
            if memories_content and memory_turn is not None and memory_turn < len(messages):
                # 把 <relevant_memories> 拼接到 request_messages 发给大模型的，而不是直接修改原始的 messages 数组。
                # 如果直接修改 messages，那么随着对话轮次增加，历史记录里会堆积无数个 <relevant_memories> 块。
                # 这会迅速耗尽 Token 额度。通过 copy()（相当于传值而不是传指针/引用），我们只是在当前这一回合“骗”大模型让它看到记忆，一旦回合结束，这段冗余的记忆文本就被丢弃了，保持了主干历史 messages 的干净。
            ...
            # --- LOG PARENT API CALL ---
            req_payload = {"model": MODEL, "system": system, "messages": request_messages, "tools": TOOLS, "max_tokens": 8000}
            ...
            if ("prompt_too_long" in str(e).lower() or "too many tokens" in str(e).lower()) and reactive_retries < MAX_REACTIVE_RETRIES:
                ...
                log_api_call("REACTIVE COMPACT TRIGGERED", {"error": str(e)})
