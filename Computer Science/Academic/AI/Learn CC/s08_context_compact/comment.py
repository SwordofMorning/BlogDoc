#!/usr/bin/env python3
"""
s08_context_compact.py - Context Compact with Full API & Event Logging
...
Logs:
  + Saves exact LLM requests & responses to agent.log
  + Logs explicit compaction events (L3 persist, L4 summary, reactive fallback, compact tool)
"""

import ast, datetime, json, os, subprocess, time
...
# ═══════════════════════════════════════════════════════════
#  Skill Registry & System Prompt Construction
# ═══════════════════════════════════════════════════════════
...
def load_skill(name: str) -> str:
    ...
    print(f"\n\033[32m[Skill Loaded: {name}]\033[0m")
    log_api_call("SKILL LOADED", {"skill_name": name, "length": len(skill.get("content", ""))})
...
# ═══════════════════════════════════════════════════════════
#  FROM s06-s07: Subagent with API Logging
# ═══════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════
#  NEW in s08: Four-Layer Compaction Pipeline with Event Logging
# ═══════════════════════════════════════════════════════════
...
def persist_large_output(tool_use_id, output):
    ...
    log_api_call("PERSIST LARGE OUTPUT (L3)", {
...
def summarize_history(messages):
    ...
    log_api_call("POST LLM CALL - SUMMARIZE (L4)", response)
...
def compact_history(messages):
    ...
    log_api_call("COMPACT HISTORY (L4 FINISHED)", {
        ...
        "before_estimated_size": before_size,
        "after_estimated_size": estimate_size(compacted),
        "summary": summary
    })
    return compacted
    ...
    while True:
        ...
        for block in response.content:
            ...
            if block.name == "compact":
                log_api_call("COMPACT TOOL EXECUTED", {"focus": block.input.get("focus", "")})
                messages[:] = compact_history(messages)
                results.append({"type": "tool_result", "tool_use_id": block.id,
